...
public class LocalDateTimeAxis extends Axis<LocalDateTime> {
    private final static ZoneOffset CURRENT_OFFSET = OffsetDateTime.now().getOffset();
     ...
     */
    public LocalDateTimeAxis(LocalDateTime lowerBound, LocalDateTime upperBound) {
        this();
     ...
     * @param upperBound The upper bound.
     */
    public LocalDateTimeAxis(String axisLabel, LocalDateTime lowerBound, LocalDateTime upperBound) {
        ...
        if (animating) {
            ...
            currentAnimationID = animator.animate(
                    new KeyFrame(Duration.ZERO,
                            new KeyValue(currentLowerBound, oldLowerBound.toInstant(CURRENT_OFFSET).toEpochMilli()),
                            new KeyValue(currentUpperBound, oldUpperBound.toInstant(CURRENT_OFFSET).toEpochMilli())
                    ),
                    new KeyFrame(Duration.millis(700),
                            new KeyValue(currentLowerBound, lower.toInstant(CURRENT_OFFSET).toEpochMilli()),
                            new KeyValue(currentUpperBound, upper.toInstant(CURRENT_OFFSET).toEpochMilli())
                    )
            );
    ...
    public double getDisplayPosition(LocalDateTime date) {
        ...
        double d = (date.toInstant(CURRENT_OFFSET).toEpochMilli() - currentLowerBound.get()) / diff;
    ...
    public LocalDateTime getValueForDisplay(double displayPosition) {
        ...
        long valueForDisplayInMilliSeconds;
        if (getSide().isVertical()) {
...
//            return new Date((long) ((displayPosition - getZeroPosition() - getHeight()) / -range * diff + currentLowerBound.get()));
            valueForDisplayInMilliSeconds = (long) ((displayPosition - getZeroPosition() - getHeight()) / -range * diff + currentLowerBound.get());
            return LocalDateTime.ofInstant(Instant.ofEpochMilli(valueForDisplayInMilliSeconds),ZoneOffset.systemDefault());
        } else {
...
//            return new Date((long) ((displayPosition - getZeroPosition()) / range * diff + currentLowerBound.get()));
            valueForDisplayInMilliSeconds = (long) ((displayPosition - getZeroPosition()) / range * diff + currentLowerBound.get());
            return LocalDateTime.ofInstant(Instant.ofEpochMilli(valueForDisplayInMilliSeconds), ZoneOffset.systemDefault());
        }
    }
    ...
    public boolean isValueOnAxis(LocalDateTime date) {
        return date.toInstant(CURRENT_OFFSET).toEpochMilli() > currentLowerBound.get() && date.toInstant(CURRENT_OFFSET).toEpochMilli() < currentUpperBound.get();
    }

    @Override
    public double toNumericValue(LocalDateTime date) {
        return date.toInstant(CURRENT_OFFSET).toEpochMilli();
    }

    @Override
    public LocalDateTime toRealValue(double v) {
        return LocalDateTime.ofInstant(Instant.ofEpochMilli((long) v),ZoneOffset.systemDefault());
    }
    ...
    protected List<LocalDateTime> calculateTickValues(double v, Object range) {
        ...
        List<LocalDateTime> previousDateList = new ArrayList<LocalDateTime>();
     ...
     */
    private List<LocalDateTime> makeDatesEven(List<LocalDateTime> dates, Calendar calendar) {
        // If the dates contain more dates than just the lower and upper bounds, make the dates in between even.
        if (dates.size() > 2) {
            List<LocalDateTime> evenDates = new ArrayList<LocalDateTime>();
            ...
            calendar.setTime(Date.from(dates.get(i).atZone(ZoneId.systemDefault()).toInstant()));
                switch (actualInterval.interval) {
                    case Calendar.YEAR:
